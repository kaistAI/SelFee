...
def generate_with_enforced_revisions(model, tokenizer, prompt: str, temperature: float, max_new_tokens: int, max_num_revisions: int, device):
    ...
    # revision_not_needed_texts = ["Revision is not needed", "\nRevision is not needed", "revision is not needed", "\nrevision is not needed", "No revision is needed", "\nNo revision is needed", "no revision is needed", "\nno revision is needed", "No revisions needed", "\nNo revisions needed", "no revisions needed", "\nno revisions needed"]
    ...
    for i in range(max_new_tokens):
        ...
        if i % stream_interval == 0 or i == max_new_tokens - 1:
            ...
            if len(output.split('### Revision')) < max_num_revisions and match_revision_not_needed_text is not None:
                ...
                # print("!!!", output_ids_string, revision_not_needed_pos)
                # print("###", match_revision_not_needed_text_ids_string)
...
def get_model_answers(model_path, model_id, question_jsons, max_num_revisions):
    ...
    for i, line in enumerate(tqdm(question_jsons)):
        ...
        input_ids = tokenizer([prompt]).input_ids
        output_ids = generate_with_enforced_revisions(
            model = model,
            tokenizer = tokenizer,
            prompt = prompt,
            temperature= 0, 
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model-path", type=str, default="ckpt/unicorn_106k_3epoch_sharegpt_alpaca_flan_merged_ver3_2048_add_mathcode_multiturn_final")
    parser.add_argument("--model-id", type=str, default="selfee")
    parser.add_argument("--question-file", type=str, default="fastchat_outdated/eval/table/question.jsonl")
    parser.add_argument("--answer-file", type=str, default="fastchat_outdated/eval/table/answer/selfee_13b_revision2")
    parser.add_argument("--num-gpus", type=int, default=2)
    parser.add_argument("--max-num-revisions", type=int, default=2)
